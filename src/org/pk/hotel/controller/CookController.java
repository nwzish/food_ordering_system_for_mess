	...
	public void doPostAction(String action, HttpServletRequest request,
		...
		try {
			...
			else if(action.equals("makeOrder"))
			{
				...
				{
					int curItem = Integer.parseInt(itemId[i]);
					int quantity = Integer.parseInt(qnt[i]);
					int max_quantity = cService.getItemCount(curItem);
					System.out.println(curItem + "  " + quantity + "  " + max_quantity);
					cService.updateItemCount(curItem, max_quantity-quantity);
				}
